import { getCurrentWeatherMock, getForecastWeatherMock, getGeoLocationMock, getWeatherStateMock } from 'src/app/mock-api/mock-data';
...
/* Location  */
  ...
  describe('LocationActions selected', () => {
    it('should update the state', () => {
      const action = LocationActions.selected({ selectedLocation: {
        name: 'Amsterdam',
        country: 'Netherlands',
        lat: 52.374,
        lon: 4.8897,
  ...
  describe('WeatherActions currentWeatherRequested', () => {
    it('should update the state', () => {
      const action = WeatherActions.currentWeatherRequested({ location: { lat: 52.374, lon: 4.8897 }});
      const state = weatherReducer(initialState, action);
  ...
  describe('WeatherActions currentWeatherRequestFailure', () => {
    it('should update the state', () => {
      const action = WeatherActions.currentWeatherRequestFailure({ error: 'Something went wrong'});
      const state = weatherReducer(initialState, action);
  ...
  describe('WeatherActions forecastWeatherRequested', () => {
    it('should update the state', () => {
      const action = WeatherActions.forecastWeatherRequested({ location: { lat: 52.374, lon: 4.8897 }});
      const state = weatherReducer(initialState, action);
  ...
  describe('WeatherActions forecastWeatherRequestSuccess', () => {
    it(('should update the state'), () => {
      const action = WeatherActions.forecastWeatherRequestSuccess({ response: getForecastWeatherMock });
      const state = weatherReducer(initialState, action);
  ...
  describe('WeatherActions forecastWeatherRequestFailure', () => {
    it('should update the state', () => {
      const action = WeatherActions.forecastWeatherRequestFailure({ error: 'Something went wrong'});
      const state = weatherReducer(initialState, action);
